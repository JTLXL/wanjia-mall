...
public class SmsUtils {
    ...
    /**
     * 存入redis key的前缀，防止与其他微服务加入的key重复(当然也可以写在配置文件)
     */
    ...
    public SendSmsResponse sendSms(String phoneNumber, String signName, String templateCode, String templateParam) {
        ...
        if (StringUtils.isNotBlank(lastTime)) {
            long lt = Long.valueOf(lastTime);
            // 按照手机号码限流
            if (System.currentTimeMillis() - lt < SMS_MIN_INTERVAL_IN_MILLIS) {
        ...
        try {
            ...
            redisTemplate.opsForValue().set(key, System.currentTimeMillis() + "", 1, TimeUnit.MINUTES);
            return resp;
